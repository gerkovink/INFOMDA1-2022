...
```{r deaths over time plots,fig.align='center'}
...
```
```{r accidents over time plots,fig.align='center'}
...
```{r Engine Types used over time,fig.align='center'}
data %>%subset(AircraftType=="Airplane" ) %>% group_by(Year,EngineType) %>% 
  ...
  theme_minimal() 
```
```{r Number of engines in machines over time,fig.align='center'}
...
```{r third time plot,fig.align='center'}
...
data %>% subset(Year %in% c("2010","2011","2012")) %>% 
  ...
  ylim(c(0,0.45))+ facet_wrap(~Year)+ylab("Proportion")+xlab("")+
  ggtitle("Accidents in different flight phases (2010-2012)")
```



```{r final 2,fig.align='center'}
data %>% subset(Year %in% c("2010","2011","2012")) %>% 
...
```{r fatality plot,fig.align='center'}
...
```{r Risks involved in fatal accident occurance,fig.align='center'}
...
```{r Risks involved in non-fatal accident occurance,fig.align='center'}
