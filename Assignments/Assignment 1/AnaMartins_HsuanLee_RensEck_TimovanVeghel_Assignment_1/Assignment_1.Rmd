...
#First a function to use for splitting up the different dates
...
#Second, a function to use when splitting a specific number from a data point, as used for both the address and the community boards
...
#Lastly, the actual cleaning pipe
...
rat_dataset <-
  ...
  mutate(Incident_Address = replace_na(Incident_Address, "Unspecified None")) %>% 
  mutate(Street_Name = replace_na(Street_Name, "None")) %>% 
  mutate(Address_Number = get.number(Incident_Address, Street_Name), Community_Board_Number = get.number(Community_Board, Borough)) %>% 
  select(Unique_Key, Due_Year, Incident_Address, Street_Name, Address_Number, Community_Board_Number)


##Regel voor Incident_Address, regel voor Community_Board, regel voor weghalen nu overbodige date columns (misschien voor de address aanpassingen)
