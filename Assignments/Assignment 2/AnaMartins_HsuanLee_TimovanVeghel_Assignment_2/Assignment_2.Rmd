...
```{r}
# Extract four movies from the old data, for better imputation results
set.seed(2022)
...
```{r}
aggr_plot <- aggr(upcoming, col=c('navyblue','red'), numbers=TRUE, sortVars=TRUE, labels=names(data), cex.axis=.7, gap=3, ylab=c("Histogram of missing data","Pattern"))
```

```{r}
imp <- mice(upcoming,m=5,maxit=50,meth='cart',seed=500)
summary(imp)
```
