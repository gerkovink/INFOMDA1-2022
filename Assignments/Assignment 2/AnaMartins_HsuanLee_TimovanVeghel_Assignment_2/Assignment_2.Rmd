...
metacriticRating <- c(64, 67, 74, 60, 86, 81, 89, 65, NA, NA, NA, NA, NA)
runtime <- c(129, 161, 106, 138, 151, 139, 117, 117, 190, NA, NA, 128, 188)
year <-
  c(2022,
    2022,
    2022,
...
actors <-
  c(
    ...
    "Brad PittMargot RobbieJean Smart"
  )
genres <-
  c(
...
contentRating <- c("PG-13", "M/12", "R", "PG-13", "PG-13", "PG-13", "PG", "R", NA, NA, "PG", "R", NA)
```
...
upcoming$metacriticRating[which(!(
  upcoming$metacriticRating %in% unique(upcoming$metacriticRating)
))] <- NA
```
...
```{r}
upcoming_pred <- predict(boost_cv, newdata = upcoming, na.rm = TRUE)
